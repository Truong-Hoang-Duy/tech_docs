...
> Mốc SHA: `bookforge@ebd0693` (origin/dev) — đối chiếu **2026-09-14**; §6 cập nhật convert-tại-chỗ có cache, câu đúng/sai không khoá, và diện phủ Cấp 3.
...
+ **Sinh câu hỏi bằng AI** (`/generate`) và **chuyển đổi dạng câu hỏi** (`/convert`) — có validator ép công thức toán phải ở dạng LaTeX hợp lệ. Convert đổi loại **ngay trên thẻ gốc** (không tạo thẻ mới); mỗi bản đã từng đổi được nhớ lại trong metadata (`type_versions`), nên đổi qua rồi đổi lại một dạng đã từng đổi **dùng lại bản cũ, không tốn lượt AI** (`restored_ids` trong response). Duyệt (approve) một thẻ trắc nghiệm/đúng-sai/trả lời ngắn/nối cột thiếu đáp án bị chặn.
...
+ Câu trắc nghiệm/đúng-sai/tự luận **không có đáp án in kèm vẫn được lưu** thay vì bị bỏ (đúng-sai lưu từng ý với `is_true=null`), chỉ ghi nhận là thiếu khoá.
...
+ Dữ liệu khung dựng sẵn nạp qua migration cho các môn Chương trình 2018; **khung kiến thức Cấp 3 (lớp 10–12) đã seed gần như đủ môn THPT** (Toán, Ngữ văn, Tiếng Anh Global Success, Sinh học, Vật lý, Hoá học, Lịch sử, Địa lý, GDKTPL, GDQP-AN, Công nghệ, Tin học, Âm nhạc, Mĩ thuật, GDTC, HĐTN — `data/curriculum/*_kntt_1{0,1,2}.json` / `*_gs_1{0,1,2}.json`); khung năng lực Cấp 3 seed cho phần lớn môn cùng danh sách.
